-- =====================================================
-- EXTRAÇÃO COMPLETA DE DADOS DOS PRODUTOS (VERSÃO CORRIGIDA)
-- =====================================================
...
-- =====================================================
-- 4. CRIAR TABELA DE COMPOSIÇÕES (CAMPO MAIOR)
-- =====================================================
CREATE TABLE IF NOT EXISTS compositions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name TEXT UNIQUE NOT NULL,  -- Mudado para TEXT para suportar composições longas
    materials TEXT[],
    percentages INTEGER[],
    properties JSONB DEFAULT '{}',
    created_at TIMESTAMP DEFAULT NOW()
...
-- Popular composições (limitando tamanho)
INSERT INTO compositions (name, materials, properties)
SELECT DISTINCT ON (composition)
    LEFT(composition, 1000) as name,  -- Limita a 1000 caracteres
    string_to_array(LEFT(composition, 500), ', ') as materials,
    jsonb_build_object(
        ...
        'thread_count', 
            CASE 
                ...
                ELSE NULL
            END,
        'is_complex', LENGTH(composition) > 200
    ) as properties
FROM (
    ...
    AND LENGTH(attributes->>'composition') < 1000  -- Ignora composições muito longas
) compositions
ON CONFLICT (name) DO NOTHING;
...
SELECT DISTINCT
    ...
    c.id
FROM products p
JOIN compositions c ON c.name = LEFT(p.attributes->>'composition', 1000)
WHERE p.attributes->>'composition' IS NOT NULL 
AND p.attributes->>'composition' != ''
AND p.attributes->>'composition' != 'None'
AND LENGTH(p.attributes->>'composition') < 1000
ON CONFLICT DO NOTHING;
