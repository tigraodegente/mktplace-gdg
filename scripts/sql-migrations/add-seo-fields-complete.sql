-- =====================================================
-- ADICIONAR CAMPOS SEO FALTANTES NA TABELA PRODUCTS
-- =====================================================
...
UPDATE products 
SET canonical_url = 'https://www.sualoja.com.br/produto/' || slug
...
SET og_description = COALESCE(
    LEFT(meta_description, 160),
    LEFT(description, 160),
    'Confira este produto incrível no Marketplace GDG'
)
WHERE og_description IS NULL;

-- OG Image - usar primeira imagem do produto se disponível
UPDATE products p
SET og_image = pi.image_url
FROM product_images pi
WHERE p.og_image IS NULL 
  AND pi.product_id = p.id 
  AND pi.is_primary = true
  AND pi.image_url IS NOT NULL;

-- Se não tiver imagem primária, usar qualquer imagem do produto
UPDATE products p
SET og_image = pi.image_url
FROM (
