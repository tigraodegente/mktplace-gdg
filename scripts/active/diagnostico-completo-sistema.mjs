...
console.log('🔍 DIAGNÓSTICO COMPLETO DO SISTEMA MARKETPLACE');
...
async function diagnosticoCompleto() {
    try {
        ...
        console.log(`   Com categoria (product_categories): ${stats.produtos_com_categoria} (${((stats.produtos_com_categoria/stats.total_produtos)*100).toFixed(1)}%)`);
        ...
        console.log(`   Produtos na tabela product_categories: ${catRel.produtos_na_tabela_nova}`);
        ...
        if (stats.produtos_com_categoria < stats.total_produtos * 0.9) {
            problemas.push(`❌ ${stats.total_produtos - stats.produtos_com_categoria} produtos sem categoria`);
        }
        ...
        if (stats.produtos_com_categoria < stats.total_produtos) {
