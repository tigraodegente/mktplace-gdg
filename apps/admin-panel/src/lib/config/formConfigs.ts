...
export const FormConfigs: Record<string, FormConfig> = {
  ...
  variacoes: {
    title: 'Variação',
    entityName: 'variação',
    subtitle: 'Gerencie variações de produtos (cores, tamanhos, etc.)',
    
    // API endpoints
    createEndpoint: '/api/variations',
    updateEndpoint: (id: string) => `/api/variations/${id}`,
    loadEndpoint: (id: string) => `/api/variations/${id}`,
    ...
    tabs: [
      {
        ...
        label: 'Básico',
        icon: 'Package',
        component: 'VariationBasicTab',
        description: 'Informações da variação'
      },
      {
        id: 'pricing',
        label: 'Preços',
        icon: 'DollarSign',
        component: 'VariationPricingTab', 
        description: 'Valores e custos'
      },
      {
        id: 'inventory',
        label: 'Estoque',
        icon: 'Archive',
        component: 'VariationInventoryTab',
        description: 'Quantidade e peso'
      },
      {
        id: 'options',
        label: 'Opções',
        icon: 'Settings',
        component: 'VariationOptionsTab',
        description: 'Cores, tamanhos, etc'
      }
    ],
    ...
    // IA não habilitada para variações (por enquanto)
    aiEnabled: false,
    ...
    onAfterLoad: (data: any) => {
      ...
      // Garantir que array de option_value_ids exista
      if (!Array.isArray(data.option_value_ids)) {
