<script lang="ts">
	...
	let { isOpen = false, productName = '', formData = {}, onClose } = $props();
	...
	async function startAnalysis() {
		if (!formData.name || formData.name.trim() === '') {
			error = 'Por favor, insira um nome para o produto antes de analisar com IA';
			return;
		}
		...
		try {
			...
			error = err.message || 'Erro ao analisar produto com IA';
