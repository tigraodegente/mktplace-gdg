<script lang="ts">
  ...
  async function handleSave() {
    ...
    saving = true;
    try {
      const response = await api.post('/shipping-modalities', formData);
      
      if (response.success) {
        toast.success('Modalidade criada com sucesso!');
        goto('/modalidades-frete');
      } else {
        toast.error(response.error || 'Erro ao criar modalidade');
      }
    } catch (error) {
...
<svelte:head>
  <title>Nova Modalidade de Frete - Admin</title>
</svelte:head>
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-gray-600 mt-1">Crie uma nova modalidade de entrega</p>
        </div>
