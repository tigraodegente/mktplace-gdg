<script lang="ts">
  ...
  onMount(async () => {
    console.log('Carregando tarifa ID:', id);
    // Simular carregamento dos dados
    formData = {
      zone_name: 'Sudeste',
      zone_states: 'SP, RJ, MG, ES',
      min_weight: 0,
      max_weight: 5,
      base_price: 15.90,
      price_per_kg: 3.50,
  ...
  async function handleSave() {
    loading = true;
    try {
      console.log('Atualizando tarifa:', formData);
      toast.success('Tarifa base atualizada com sucesso!');
      goto('/tarifas');
    } catch (error) {
      toast.error('Erro ao atualizar tarifa base');
    } finally {
      loading = false;
    }
  }
  ...
  async function handleDelete() {
    if (confirm('Tem certeza que deseja excluir esta tarifa base?')) {
      try {
        console.log('Excluindo tarifa ID:', id);
        toast.success('Tarifa base excluída com sucesso!');
        goto('/tarifas');
      } catch (error) {
        toast.error('Erro ao excluir tarifa base');
      }
    }
  }
</script>
  ...
  <div class="bg-white border-b">
    <div class="max-w-[calc(100vw-100px)] mx-auto px-4 py-6">
      <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Editar Tarifa Base</h1>
