...
async function getPromptFromDatabase(promptName: string, currentData: any, categories: any[] = [], brands: any[] = []): Promise<string | null> {
	try {
		...
		processedTemplate = processedTemplate.replace(/\{\{condition\}\}/g, currentData.condition || 'Não informado');
		processedTemplate = processedTemplate.replace(/\{\{warranty_period\}\}/g, currentData.warranty_period || 'Não informado');
		processedTemplate = processedTemplate.replace(/\{\{care_instructions\}\}/g, currentData.care_instructions || 'Não informado');
		processedTemplate = processedTemplate.replace(/\{\{tax_class\}\}/g, currentData.tax_class || 'Não informado');
		processedTemplate = processedTemplate.replace(/\{\{manufacturing_country\}\}/g, currentData.manufacturing_country || 'Não informado');
		processedTemplate = processedTemplate.replace(/\{\{minimum_stock\}\}/g, currentData.minimum_stock || '0');
		processedTemplate = processedTemplate.replace(/\{\{delivery_days\}\}/g, currentData.delivery_days || 'Não informado');
		processedTemplate = processedTemplate.replace(/\{\{shipping_restrictions\}\}/g, currentData.shipping_restrictions || 'Não informado');
...
async function generateAISuggestionsFromDatabase(currentData: any, categories: any[], brands: any[]): Promise<Suggestion[]> {
	...
	try {
		...
		// 7. 🔧 CAMPOS AVANÇADOS
		console.log('🔧 Processando: enrich_advanced_fields');
		const advancedPrompt = await getPromptFromDatabase('enrich_advanced_fields', currentData);
		if (advancedPrompt) {
			...
			const advancedSuggestions = parseAIResponse(advancedResponse.choices[0].message.content);
			allSuggestions.push(...advancedSuggestions);
			console.log(`✅ Campos Avançados: ${advancedSuggestions.length} sugestões`);
		}
