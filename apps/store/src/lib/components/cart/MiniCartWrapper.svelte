<script lang="ts">
/**
 * MiniCart Compatibility Wrapper
 * 
 * Wrapper que garante compatibilidade total do MiniCart
 * independente da versão do store que está sendo usada
 */
...
// Import do store via features (automaticamente usa a versão ativa)
import { cartStore, getCartStoreInfo } from '$lib/features/cart';
...
// Desestruturação segura do store (compatível com ambas as versões)
...
// Log da versão do store ativa (para debugging)
$effect(() => {
  const storeInfo = getCartStoreInfo();
  console.log(`🛒 MiniCart usando: ${storeInfo.type} v${storeInfo.version}`);
});
...
// Computed - com compatibilidade para ambos os stores
const itemCount = $derived(() => {
  try {
    return $sellerGroups.reduce((sum: number, g: any) => sum + g.items.length, 0);
  } catch (error) {
    console.warn('Erro ao calcular itemCount:', error);
    return 0;
  }
});
...
const allItems = $derived(() => {
  try {
    return $sellerGroups.flatMap((g: any) => g.items);
  } catch (error) {
    console.warn('Erro ao acessar items:', error);
    return [];
  }
});
...
// Computed para totais - com compatibilidade
const hasShipping = $derived(() => {
  try {
    return ($cartTotals as any).totalShipping > 0 || ($cartTotals as any).total_shipping > 0;
  } catch (error) {
    return false;
  }
});
...
const totalSavings = $derived(() => {
  try {
    return ($cartTotals as any).totalDiscount || ($cartTotals as any).total_discount || 0;
  } catch (error) {
    return 0;
  }
});
...
async function handleRemoveItem(productId: string, sellerId: string, variant?: { color?: string; size?: string }) {
  ...
  // Remover item do carrinho - compatible call
  try {
...
function getFreeShippingProgress(): { current: number; target: number; percentage: number; remaining: number; hasFreeship: boolean } {
  ...
  let current = 0;
  try {
    current = ($cartTotals as any).cartSubtotal || ($cartTotals as any).cart_subtotal || 0;
  } catch (error) {
    current = 0;
  }
...
// Função auxiliar para acessar totais de forma compatível
function getCartTotal(): number {
  try {
    return ($cartTotals as any).cartTotal || ($cartTotals as any).cart_total || 0;
  } catch (error) {
    return 0;
  }
}
...
function getCartSubtotal(): number {
  try {
    return ($cartTotals as any).cartSubtotal || ($cartTotals as any).cart_subtotal || 0;
  } catch (error) {
    return 0;
  }
}
      ...
      <div class="bg-white p-6">
        ...
        <div class="space-y-2 mb-6">
          <div class="flex justify-between text-sm" style="font-family: 'Lato', sans-serif;">
            ...
            <span class="font-medium">{formatCurrency(getCartSubtotal())}</span>
          </div>
          ...
          <div class="flex justify-between text-lg font-bold border-t pt-2" style="font-family: 'Lato', sans-serif;">
            <span>Total:</span>
            <span class="text-[#00BFB3]">{formatCurrency(getCartTotal())}</span>
          </div>
          ...
          <p class="text-xs text-gray-500 text-center" style="font-family: 'Lato', sans-serif;">
            ou até {pricingConfig?.installments_default || 12}x de {formatCurrency(
              getCartTotal() > 0 && (pricingConfig?.installments_default || 12) > 0
                ? getCartTotal() / (pricingConfig?.installments_default || 12)
                : 0
            )}
          </p>
        </div>
